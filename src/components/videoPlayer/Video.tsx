		...
		if (video) {
			video.addEventListener("loadeddata", () => {
				...
				if (trackChaptersRef.current) {
					...
					if (cues) {
						payload["chapters"] = (Object.values(cues) as VTTCue[]).map(
							(cue: VTTCue, index) => ({
								index,
								...
								percentageTime: (
									((cue.endTime - cue.startTime) / video.duration) *
									100
								).toFixed(2),
							})
						);
		...
		<>
			<div onClick={onPlayPause} className='html-video-container'>
				...
				>
					...
					<track
						ref={trackMetaDataRef}
						default
						kind='metadata'
						src={videoSourceInfo.vtt}
					/>
					<track
						ref={trackChaptersRef}
						default
						kind='chapters'
						src={videoSourceInfo.chapters}
					/>
				</video>
			</div>
			{shouldhaveStats && (
